...
exports.saveEquipo = (req, res) => {
    ...
    conexion.query('INSERT INTO equipo SET ?', {codigo: codigo, nombre: nombre, directorT: directorT, eslogan: eslogan}, (error, results) => {
        if(error){
            console.log(error);
        }
    });
...
exports.saveHospedaje = (req, res) => {
    ...
    conexion.query('SELECT codHotel FROM hotel WHERE nombre = ?', [id_hotel], (error, results) => {
        if(error){
            console.log(error);
        }else{
            ...
            id_equipo = conexion.query('SELECT codigo FROM equipo WHERE nombre = ?', [id_equipo], (error, results) => {
                if(error){
                    console.log(error);
                }else{
                    id_equipo = results[0].codigo;

                    conexion.query('INSERT INTO alojamiento SET ?', {id_hotel: id_hotel, id_equipo: id_equipo, fecha_inicio: fecha_inicio, fecha_fin: fecha_fin}, (error, results) => {
                        if(error){
                            console.log(error);
                        }else{
